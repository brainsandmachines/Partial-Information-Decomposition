...
- `source_conwell_code/pressures/brain_data/`: Source Conwell analysis scripts kept inside the project tree.
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `pid_calc` (line 26) | `config=None, sources=None, target=None, rng=torch.Generator().manual_seed(56), method=None, on_rvs: callable=None, covariance: torch.Tensor=None` | `pid`, `mi` | No docstring; infer behavior from name/signature before reuse. |
| `pid_idep_wrapper` (line 51) | `config, sources=None, target=None, covariance=None, rng=None, on_rvs=None` | `pid`, `mi` | This function is a wrapper to PID calculated by Idep_multivariate_gauss class, which implements the Idep PID calculation for multivariate Gaussian variables. |
| `pid_tilde_wrapper` (line 79) | `config: dict, sources: list, target: list, covariance: torch.Tensor, rng: torch.random.Generator, on_rvs: callable=None` | `pid`, `mi` | This function is a wrapper to PID calculated by BROJA and implemented by Venkatesh et al. |
| `delta_wrapper` (line 117) | `config, sources, target, covariance, rng, on_rvs` | `pid`, `mi` | This function is a wrapper to PID calculated by BROJA and implemented by Venkatesh et al. |
| `_to_numpy_samples` (line 154) | `data` | `data` | Convert torch/numpy samples to the numpy format expected by flow-pid. |
| `flow_pid_wrapper` (line 164) | `config: dict, sources: list, target: list, covariance: torch.Tensor, rng: torch.random.Generator, on_rvs: callable=None` | `pid`, `mi` | Wrapper for flow-pid. |
...
| `compute_ridge_cv_r2` (line 25) | `X, y, alphas=None` | ridge_cv.best_score_, `ridge_cv` | Compute cross-validated R² using RidgeCV with efficient LOO cross-validation. |
| `cond_cov` (line 61) | `sigma_1, sigma_2, sigma12, sigma21` | `cond_cov` | This function will compute the conditional covariance matrix of two Gaussian variables Sigma_1\|2 = Sigma_1 - Sigma12*inv(Sigma_2)*Sigma21 input: sigma_1,sigma_2 are torch tensors of shape (d,d) d is the dimension of each observation. |
...
| `create_cov_matrix` (line 87) | `rvs: list=[], verbose=False, Sigma=None, dims: list=None, device='cpu', check_singular=True` | `cov_dict` | This function will create the covariance matrix for the three variables M1,M2,T input: M1,M2,T are torch tensors of shape (N,p) rvs is a list of the three variables [M1,M2,T] N is the number of observations, p is the dimension of each obser... |
...
| `old_para_create_cov_matrix` (line 241) | `dims, Sigmas=None, verbose=False` | `cov_dict` | This function will create the covariance matrix for the three variables M1,M2,T input: M1,M2,T are torch tensors of shape (N,p) rvs is a list of the three variables [M1,M2,T] N is the number of observations, p is the dimension of each obser... |
...
| `correlation_matrix` (line 409) | `X` | `corr_matrix`; call `array(...)` | Compute the correlation matrix of the columns of X. |
| `block_singularity_check` (line 424) | `X, tol=1e-10` | `min_eig`, min_eig <= tol; `singular_dict` | Check if a block is singular or ill-conditioned. |
| `singularity_report` (line 445) | `X_M1, X_M2, y_real, tol=1e-10, return_printing_required=False` | `report`; `report`, `printing_required` | Return min eigenvalue and singularity flag for blocks and combinations. |
...
| `anchored_oas_shrinkage` (line 1043) | `Sigma_full: torch.Tensor, cov_loo_all: torch.Tensor, n_samples: int` | call `unsqueeze(...)`, `cov_loo_all_shrunk` | Calculates OAS parameters ONCE on the full matrix, and applies the EXACT SAME linear shrinkage to all LOO matrices. |
...
| `mi_wishahrt_bias` (line 41) | `dims: list, n_samples: int` | `bias_mi`; dict (bias_mi_1_t, bias_mi_2_t, bias_mi_12, bias_tri_mi) | Calculate the bias for mutual information estimation between two Gaussian variables with given dimensions and sample size |
| `permuteation_debiased` (line 77) | `config, term='nume'` | `value` | No docstring; infer behavior from name/signature before reuse. |
| `permutation_null_debias` (line 96) | `config, func` | dict (bias, perm_mean, perm_std, perm_se, perm_values, n_perm); dict (debiased, perm_mean, perm_std, perm_se, perm_values, n_perm) | Debias an MI-like estimator by subtracting its permutation null floor. |
| `unique_bias` (line 164) | `config, functions_dict: dict=None` | `bias_dict` | No docstring; infer behavior from name/signature before reuse. |
| `bias_func` (line 184) | `config, model` | dict (i, h); dict (k, j) | No docstring; infer behavior from name/signature before reuse. |
...
| `plot_single_component_heatmap` (line 287) | `df, *, pid_ver, component_key, base_title=None, x_col='N', y_col='p', aggfunc='last', cmap='viridis', figsize=(9, 7), save_dir=None, show=True, mean_fmt='.3f', std_fmt='.3f'` | `fig`, `ax`; None | Plot one heatmap for one PID version and one component. |
...
| `calcualte_mi` (line 103) | `config, sigma_dict, term='full'` | dict (mi_tri, mi_bi_1, mi_bi_2, nume, deno); dict | This function calculates the tri-variate mutual information using the covariance matrices and the formula MI = 0.5 * (log\|deno_matrix\| - log\|nume_matrix\|) |
| `calculate_mi_raw` (line 129) | `device, sigma, dims` | dict (tri_mi, bi_mi_1_t, bi_mi_2_t) | This function calculates the tri-variate or bi-variate mutual information using the covariance matrices without any whitening - in raw mode (: Input: device: torch.device - the device on which to perform the calculations sigma: torch.Tensor... |
| `para_calcualte_mi` (line 184) | `config, sigma_dict, term='full', assumed_whitened=True` | dict (mi_tri, mi_bi_1, mi_bi_2, nume, deno); dict | This function calculates the tri-variate mutual information using the for multiple covariances matrices and the formula MI = 0.5 * (log\|deno_matrix\| - log\|nume_matrix\|) |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `Idep_multivariate_gauss.__init__` (line 30) | `self, config, rng=None, sources: Optional[list]=None, targets: Optional[list]=None, cov_matrix: Optional[torch.tensor]=None, base_e: bool=True, bias_correction: bool=False` | No explicit return; likely `None` / side effects. | Initialize the Idep multivariate gaussian class input: M1,M2,T are torch tensors of shape (N,P) N is the number of observations P is the number of variables in each observation config is a dictionary containing the configuration for the PID... |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `pid_simulation` (line 22) | `config, rng, cov, pid_ver, true_values=None` | dict (unq1, unq2, syn, red, mi_tri, mi_bi_1) | Run PID simulation with know ground truth PID values from the covariance matrix sample for the true covariance matrix, calculate the PID using the specified method, and return the results along with the ground truth PID values calculated fr... |
...
| `Idep_univariate_gauss.idep` (line 197) | `self, cov_matrix: Optional[torch.tensor]=None` | Annotated: `dict` | This function will compute the full Idep PID decomposition input: cov_matrix is a torch tensor of shape (d,d) in case you want to provide a different covariance matrix output: a dictionary with the PID values keys: 'red', 'unq0', 'unq1', 's... |
...
| `check_idep_gauss_p0_example` (line 323) | `q=0.3, r=0.5, tol=1e-08` | No explicit return; likely `None` / side effects. | Example 3 from the paper: p = corr(X0, X1) = 0 q = corr(X0, Y) != 0 r = corr(X1, Y) != 0 Expected: unq0 = 1/2 log((1 - q^2 r^2) / (1 - q^2)) unq1 = 1/2 log((1 - q^2 r^2) / (1 - r^2)) red = 1/2 log(1 / (1 - q^2 r^2)) syn = 1/2 log(((1 - q^2)... |
...
| `para_Idep_multivariate_gauss.idep` (line 195) | `self, cov_matrix: Optional[torch.tensor]=None` | Annotated: `dict` | This function will compute the full Idep PID decomposition input: cov_matrix is a torch tensor of shape (d,d) in case you want to provide a different covariance matrix output: a dictionary with the PID values keys: 'red', 'unq1', 'unq2', 's... |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `on_covariance` (line 5) | `config, data` | dict (cov) | This will call an intermidate function on the covariance Input: data: a tuple containing the covariance matrix and a list of random variables config: a dictionary with data and parameters for the function to apply on the covariance matrix. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `mean_std_csv_results` (line 23) | `results_dict` | `mean_results`, `std_results` | Helper: Compute mean results across seeds |
| `m7_m8_mean_std_csv_results` (line 31) | `results_dict` | `mean_results`, `std_results` | Helper: Compute mean results across seeds |
...
| `to_python_scalar` (line 92) | `value` | `value`; call `tolist(...)`; call `item(...)` | Convert torch/numpy scalar values into normal Python scalars. |
...
| `already_exists_in_csv` (line 213) | `output_folder, N, p, pid_ver, seed, csv_title='pid_results'` | call `any(...)`; False | Check whether this exact simulation setting already exists in the CSV file specific to pid_ver. |
...
| `extract_num_den_matrices` (line 404) | `config: dict, matrix: torch.tensor` | `nume_matrix`, `denoq_matrix`, `denor_matrix` | Extract the numerator and denominator covariance matrices for M7/M8 from the full covariance matrix. |
...
| `_to_scalar` (line 676) | `value` | call `float(...)`; None; call `item(...)` | No docstring; infer behavior from name/signature before reuse. |
| `_build_pid_rows_from_node` (line 684) | `node_rows, known_component` | `pid_rows`, `pid_ground_truth` | Build PID rows from node summaries by filling missing components. |
| `plot_pid_trajectory_vs_p_over_N` (line 733) | `results, ground_truth=None, save_path=None, title='PID components vs p/N', p_col='p', n_col='N', components=None, figsize=(10, 6), dpi=300, descending_x=True` | `traj`; `p`; None; call `sum(...)`; ... | Plot PID components and mutual information terms as a function of p/N. |
| `plot_pid_trajectory_vs_p_over_N.get_total_p` (line 790) | `p` | `p`; call `sum(...)` | No docstring; infer behavior from name/signature before reuse. |
...
| `sort_m7_m8_results` (line 257) | `results_list` | `m7_results_list`, `m8_results_list` | Helper: Sort results list by N and p values for sperate by m7 and m8. |
...
| `sort_m7_m8_results` (line 335) | `results_list` | list, list | Helper: Sort results list by N and p values for sperate by m7 and m8. |
...
| `oracle_shrinkage_cov` (line 14) | `X, assume_centered=False, return_shrinkage=False` | oas.covariance_; oas.covariance_, oas.shrinkage_ | Oracle Approximating Shrinkage covariance estimator. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `_build_whitened_blocks_from_cov` (line 16) | `S, n0, n1, n2` | `P`, `Q`, `R`, `S_dict` | Given a covariance matrix S in block order [X0, X1, Y], return the whitened blocks P, Q, R using the same helpers as the main code. |
| `make_random_true_cov` (line 30) | `config: dict, rng: torch.Generator \| None=None` | `true_cov_m8`, `true_cov_m7` | Construct a whitened Gaussian M8 covariance and its corresponding M7 covariance. |
...
| `create_m7_cov` (line 293) | `config: dict, cov_m8, whitening_normalize: bool=True` | call `cat(...)` | Takes covariance of M8 and creates M7 out of M8 |
...
| `bias_resampling` (line 51) | `config: dict, calc_func: callable=None` | Annotated: `dict` | This function will calculate the statistics value and it's and will return a dictionary with the following keys: Input: data: the data to calculate the statistic on (covariance etc ) config: a dictionary with data return: results_dict[stati... |
...
| `creature_featurs` (line 46) | `rng, snr, unique_ratio, features, signal, redundant_dim=None` | `X_M1`, `X_M2`, `target` | No docstring; infer behavior from name/signature before reuse. |
...
|---|---|---|---|
| `get_run_config` (line 20) | `No inputs.` | Annotated: `dict` | No docstring; infer behavior from name/signature before reuse. |
| `half_permute` (line 36) | `rng, features, snr=10` | `X_M1`, `X_M2` | No docstring; infer behavior from name/signature before reuse. |
| `orthogonal_vectors` (line 57) | `rng, n, p, features, noise=None, singal=None, unique_ratio=None, function=None` | `X_M1`, `X_M2`, `target` | No docstring; infer behavior from name/signature before reuse. |
| `feature_creation` (line 98) | `rng, unique_ratio, unique_method='orthogonal', n=1024, p=100, mixing_dimension=None, snr=10.0, method='standard', show_diagnostic_plots=False` | `X_M1`, `X_M2`, `y_real` | Creates dummy predictors and a target Args: rng: Random number generator unique_ratio : number between 0 and 1, indicating the proportion of unique features in each source. |
| `test_both_unique` (line 139) | `rng, unique_ratio, n=1024, p=100, snr=10.0, method='standard'` | `ca_results`, `pid_results`, `mi_results` | No docstring; infer behavior from name/signature before reuse. |
...
| `feature_creation` (line 48) | `rng, r_str, u1_str, u2_str, unique_method='orthogonal', n=1024, p=100, mixing_dimension=None, snr=10.0, method='standard', show_diagnostic_plots=False` | `X_M1`, `X_M2`, `y_real` | Creates dummy predictors and a target Args: rng: Random number generator r_str: strength of redundant features u1_str: strength of unique features in source 1 u2_str: strength of unique features in source 2 n: Number of samples p: Number of... |
| `test` (line 86) | `rng, r_str, u1_str, u2_str, n=1024, p=100, snr=10.0, method='standard'` | `ca_results`, `betas_dict`, `pid_results`, `mi_results` | No docstring; infer behavior from name/signature before reuse. |
...
Random-variable story examples and Flow-PID grid-search tooling.

### `Simulations/Theoretical_Examples/RVs_Story/flow_pid_grid_search.py`

File description: Standalone Flow-PID hyperparameter grid search using k-fold synergy error.
...
| `load_example_and_truth` (line 70) | `example_name` | `generator`, `truth` | No docstring; infer behavior from name/signature before reuse. |
...
| `standardize_train_val` (line 98) | `train, val` | (train - mean) / std, (val - mean) / std | No docstring; infer behavior from name/signature before reuse. |
...
| `parse_args` (line 252) | `No inputs.` | call `parse_args(...)` | No docstring; infer behavior from name/signature before reuse. |
...
File description: Python module for all reg examples-related project logic.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `_as_float` (line 20) | `value` | float(value.detach().cpu().numpy()) if isinstance(value, torch.Tensor) else float(value) | No docstring; infer behavior from name/signature before reuse. |
| `csv_path` (line 24) | `results_dir: Path, example: str, method: str` | results_dir / f'{example_name}_{method_name}_seeds.csv' | No docstring; infer behavior from name/signature before reuse. |
| `csv_has_seed` (line 30) | `path: Path, seed: int` | False; call `any(...)` | No docstring; infer behavior from name/signature before reuse. |
| `seed_is_done` (line 37) | `seed: int, example_name: list, results_dir: Path` | call `all(...)` | No docstring; infer behavior from name/signature before reuse. |
| `loop_examples` (line 45) | `config: dict, function_to_run: list, example_name: list, save_image: bool=True` | `all_results` | Loop over all suppresion examples and run them |
| `save_seed_csvs` (line 59) | `seed: int, all_results: dict, results_dir: Path` | No explicit return; likely `None` / side effects. | Save current seed values into one CSV per example and PID method. |
| `mean_results_from_csvs` (line 77) | `results_dir: Path, example: str, seeds: list[int]` | `results` | Build an averaged PID result dict from saved seed CSVs. |
| `loop_examples_over_seeds` (line 97) | `config: dict, function_to_run: list, example_name: list, num_seeds: int=None, seeds: list=None` | `mean_by_example` | Run all examples over seeds, save seed CSVs, then save averaged tables. |

### `Simulations/Theoretical_Examples/RVs_Story/regular_examples/core_model.py`

File description: Shared runner and ground-truth calculation for regular RV story examples.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `true_mi_pid` (line 19) | `sources, target, covariance=None` | `pid`, `mi` | Because we know Unique 2 is zero, We can know the true values of the PID components, As well because we know the it all gaussian we can calculate the true MI values as well. |
| `main_func` (line 52) | `config, function_to_run` | `results_dict` | No docstring; infer behavior from name/signature before reuse. |

### `Simulations/Theoretical_Examples/RVs_Story/regular_examples/equal_unique.py`

File description: Equal-unique random-variable generator and standalone example runner.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `equal_unique` (line 17) | `rng, n, p, noise_std` | `X_1`, `X_2`, `y` | Expectation: unq1 = unq2 redundancy > 0 synergy = 0 |
...
File description: Python module for all supp examples-related project logic.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `_as_float` (line 22) | `value` | float(value.detach().cpu().numpy()) if isinstance(value, torch.Tensor) else float(value) | No docstring; infer behavior from name/signature before reuse. |
| `csv_path` (line 26) | `results_dir: Path, example: str, method: str` | results_dir / f'{example_name}_{method_name}_seeds.csv' | No docstring; infer behavior from name/signature before reuse. |
| `csv_has_seed` (line 32) | `path: Path, seed: int` | False; call `any(...)` | No docstring; infer behavior from name/signature before reuse. |
| `seed_is_done` (line 39) | `seed: int, example_name: list, results_dir: Path` | call `all(...)` | No docstring; infer behavior from name/signature before reuse. |
| `loop_examples` (line 47) | `config: dict, function_to_run: list, example_name: list, save_image: bool=True` | `all_results` | Loop over all suppresion examples and run them |
| `save_seed_csvs` (line 61) | `seed: int, all_results: dict, results_dir: Path` | No explicit return; likely `None` / side effects. | Save current seed values into one CSV per example and PID method. |
| `mean_results_from_csvs` (line 79) | `results_dir: Path, example: str, seeds: list[int]` | `results` | Build an averaged PID result dict from saved seed CSVs. |
| `loop_examples_over_seeds` (line 99) | `config: dict, function_to_run: list, example_name: list, num_seeds: int=None, seeds: list=None` | `mean_by_example` | Run all examples over seeds, save seed CSVs, then save averaged tables. |

### `Simulations/Theoretical_Examples/RVs_Story/suppresion_examples/core_model.py`

File description: Shared runner and ground-truth calculation for suppression RV story examples.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `true_mi_pid` (line 19) | `sources, target, covariance=None` | `pid`, `mi` | Because we know Unique 2 is zero, We can know the true values of the PID components, As well because we know the it all gaussian we can calculate the true MI values as well. |
| `main_func` (line 51) | `config, function_to_run` | `results_dict` | No docstring; infer behavior from name/signature before reuse. |

### `Simulations/Theoretical_Examples/RVs_Story/suppresion_examples/full_suppresion.py`

File description: Python module for full suppresion-related project logic.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `full_suppresion` (line 17) | `rng, n, p, noise_std` | `X_1`, `X_2`, `t` | Intended theoretical structure: Y X1 = Y + N X2 = N X2 has: - no private signal source about Y - synergistic/suppressor role through shared nuisance N So we expect: unq2 = 0 redundancy = 0 unq1 > 0 synergy > 0 under MMI-like Gaussian PID. |

### `Simulations/Theoretical_Examples/RVs_Story/suppresion_examples/unq12_zero.py`

File description: Python module for unq12 zero-related project logic.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `unq12_zero` (line 17) | `rng, n, p, noise_std` | `X_1`, `X_2`, `t` | Intended theoretical structure: Y = R + U + eps_y X1 = R + U + N X2 = R + N X2 has: - redundancy through R - no private signal source about Y - synergistic/suppressor role through shared nuisance N So we expect: unq1 = 0 unq2 = 0 redundancy... |

### `Simulations/Theoretical_Examples/RVs_Story/suppresion_examples/unq2_zero.py`

File description: Python module for unq2 zero-related project logic.

| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `unq2_zero` (line 17) | `rng, n, p, noise_std` | `X_M1`, `X_M2`, `y` | Intended theoretical structure: Y = R + U + eps_y X1 = R + U + N X2 = R + N X2 has: - redundancy through R - no private signal source about Y - synergistic/suppressor role through shared nuisance N So we expect: unq2 = 0 redundancy > 0 unq1... |

## Simulations/evil_twin

Evil-twin covariance examples and related checks.
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `empirical_covariance_matrix_torch` (line 6) | `X1, X2, T, correction=1` | call `cov(...)` | Compute the empirical covariance matrix of (X1, X2, T). |
...
File description: Seed-loop and CSV helpers for running PID_calc methods on Sonic/Shadow evil-twin samples.
...
| `method_csv_path` (line 53) | `output_dir: Path, method: str, prefix: str='evil_twin_pid'` | Annotated: `Path` | Build the output CSV path for one PID method. Task-specific CSV helper for evil-twin PID sweeps. |
| `append_rows_to_csv` (line 68) | `path: Path, rows: list[dict]` | Annotated: `Path` | Append rows to a CSV file, creating the header when needed. Uses stdlib CSV to avoid importing broad experiment utilities. |
...
| `make_pid_config` (line 106) | `n: int, p: int, device: str, flow_epochs: int, flow_verbose: bool` | Annotated: `dict` | Create the config dictionary expected by `Partial_Information_Decomposition.PID_calc.pid_calc`. Assumes equal source and target dimensions `p`. |
| `result_row` (line 131) | `seed: int, twin: str, method: str, n: int, p: int, pid: dict, mi: dict` | Annotated: `dict` | Create a successful result row for one Sonic/Shadow twin and one PID method. |
| `error_row` (line 164) | `seed: int, twin: str, method: str, n: int, p: int, error: Exception` | Annotated: `dict` | Create an error row when a PID method fails, preserving the seed/method in the method CSV. |
| `run_seed` (line 193) | `seed: int, config: dict, methods: tuple[str, ...], output_dir: Path, csv_prefix: str` | Annotated: `dict` | Generate one evil-twin sample pair with `evil_twin_example_torch`, run each requested `pid_calc` method on Sonic and Shadow, and append method CSV rows immediately. |
...
| `mean_summary_rows` (line 254) | `seed_results: dict, methods: tuple[str, ...], twins: tuple[str, ...]=SUMMARY_TWINS` | Annotated: `list[dict]` | Calculate mean PID and MI values across seeds for each method, with Sonic and Shadow summarized separately. |
...
| `summary_rows_to_pid_results` (line 317) | `rows: list[dict], twin: str` | Annotated: `dict` | Convert mean summary rows to the PID result shape used by RVs_Story table images. |
| `save_summary_table_images` (line 346) | `rows: list[dict], output_dir: Path, prefix: str, config: dict, twins: tuple[str, ...]=SUMMARY_TWINS` | Annotated: `list[Path]` | Save RVs_Story-style PID comparison images for the mean summary, one image per twin. Reuses `save_pid_comparison_table`. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `load_roi_data` (line 20) | `args, roi_name='V1v'` | `lh_roi_fmri`, `rh_roi_fmri`, `train_img_list`, `training_img_dir` | No docstring; infer behavior from name/signature before reuse. |
...
| `load_data_algonauts` (line 41) | `paths_dict, args=None, subj=1, plot_fmri=False` | `output_dict`, `data_dict` | Load fMRI data and image file lists for a given subject. |

### `encoding_model/commonality.py`

File description: Commonality and variance partitioning calculations for encoding models.
...
| `commonality_analysis` (line 28) | `features_X1, features_X2, target, method='standard', alphas=None, scale_by_target_variance=False, **_ignored_kwargs` | dict (R²_X1, R²_X2, R²_X12, unique_X1, unique_X2, common) | Decompose predictive power into unique, common, and unexplained components. |
...
| `split_dataset` (line 128) | `train_img_list, test_img_list, rand_seed=5, train_p=90` | `idxs_train`, `idxs_val`; `idxs_train`, `idxs_val`, `idxs_test` | No docstring; infer behavior from name/signature before reuse. |
| `fmri_data_loader` (line 153) | `lh_fmri, rh_fmri, train_img_list, test_img_list, train_img_dir, test_img_dir, batch_size=500, train_p=90` | `fmri_dict`, `train_imgs_dataloader`, `val_imgs_dataloader` | No docstring; infer behavior from name/signature before reuse. |
...
| `roi_fmri_data` (line 240) | `args, lh_fmri, rh_fmri` | `roi_names`, `lh_roi_fmri`, `rh_roi_fmri` | Map fMRI data to ROIs. |
| `get_specific_roi_fmri` (line 294) | `args, lh_fmri, rh_fmri, roi_name` | lh_roi_fmri[roi_index], rh_roi_fmri[roi_index] | Get fMRI data for a specific ROI. |
...
| `encoding_model.extract_features` (line 51) | `self, dataloader, pca` | call `vstack(...)` | No docstring; infer behavior from name/signature before reuse. |
| `encoding_model.train` (line 61) | `self, train_data_loader, lh_fmri_train, rh_fmri_train, features_train: Optional[np.ndarray]=None, alphas: Optional[np.ndarray]=None` | self.reg_lh, self.reg_rh | No docstring; infer behavior from name/signature before reuse. |
| `encoding_model.validate` (line 82) | `self, reg_lh, reg_rh, lh_fmri_val, rh_fmri_val, features_val` | `lh_correlation`, `rh_correlation` | "This funciton validates the encoding model on the validation set and returns the correlation scores for each hemisphere. |
| `encoding_model.run_model` (line 110) | `self, train_imgs_dataloader, val_imgs_dataloader, lh_fmri_train, rh_fmri_train, lh_fmri_val, rh_fmri_val, batch_size=100, ncomponents=None` | `reg_lh`, `reg_rh`, `lh_correlation`, `rh_correlation` | This function runs the entire encoding model pipeline: feature extraction, training, validation without testing. |

### `encoding_model/grid_search.py`

File description: Grid search helpers for encoding-model experiments.
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `compute_ols_cv_r2` (line 6) | `X, y, return_model=False` | ridge_cv.best_score_; `ridge_cv`, ridge_cv.best_score_ | Compute cross-validated R2 using leave-one-out cross-validation. |
| `compute_ridge_cv_r2` (line 32) | `X, y, alphas=None, return_model=False` | ridge_cv.best_score_; `ridge_cv`, ridge_cv.best_score_ | Compute cross-validated R2 using RidgeCV with efficient LOO cross-validation. |
| `compute_r2` (line 64) | `X, y, return_model=False` | `score`; `model`, `score` | Compute in-sample R2 for OLS regression. |
| `compute_lasso_cv_r2` (line 87) | `X, y` | `mo_lasso`, `r2` | Compute in-sample R2 after fitting multi-output LassoCV. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `real_model_func` (line 52) | `model, layer_name, model_path, batch_size, ncomponents, train_data_loader` | `model`, `real_reg_lh`, `real_reg_rh`, `pca`, `real_feature` | Create and train the real encoding model using fMRI data and image features. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `create_predictions` (line 12) | `reg_lh, reg_rh, features` | `y_hat_lh`, `y_hat_rh` | Create fMRI predictions using trained regression models. |
| `create_encoder` (line 28) | `rng, features, target, n_features` | `model`, `selected_features` | Create and train a linear regression encoder. |
| `permutate_models` (line 59) | `rng, features, suppression_strength` | `X_M1`, `X_M2` | No docstring; infer behavior from name/signature before reuse. |
| `noise_component` (line 80) | `rng, features, suppression_strength, permutation` | `X_M1`, `X_M2` | Create suppresion model using only noise component. |
| `create_supression_model` (line 106) | `rng, signal, suppresion_method, features, suppression_strength=0.5, snr=1.0, mixing_dimension=None` | `X_M1`, `X_M2`, `target` | Create suppression model features X_M1 and X_M2 based on the given parameters. |
...
File description: Shared CSV, path, covariance, and result utilities for PID wrapper scripts.
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `average_rdms` (line 7) | `rdm_array` | 1 - fisherz_inv(fisherz(np.stack([1 - rdm for rdm in rdm_array])).mean(axis=0, keepdims=True).squeeze()); call `arctanh(...)`; call `tanh(...)` | No docstring; infer behavior from name/signature before reuse. |
| `average_rdms.fisherz` (line 8) | `r, eps=1e-05` | call `arctanh(...)` | No docstring; infer behavior from name/signature before reuse. |
| `average_rdms.fisherz_inv` (line 11) | `z` | call `tanh(...)` | No docstring; infer behavior from name/signature before reuse. |
...
| `NSDBenchmark.get_sample_stimulus` (line 80) | `self, image_index=None` | call `open(...)` | No docstring; infer behavior from name/signature before reuse. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `get_nsd_path` (line 21) | `config=None, key='NSD_PATH'` | call `get(...)` | No docstring; infer behavior from name/signature before reuse. |
...
| `load_NSD_benchmark_ROI_metadata` (line 455) | `subjs, space, ncsnr_threshold=0.2, t_threshold=1, savedir=None, overwrite=False, **kwargs` | `voxel_metadata`; `excl_idx`; call `isin(...)`; call `logical_not(...)` | No docstring; infer behavior from name/signature before reuse. |
| `load_NSD_benchmark_ROI_metadata.get_idx_by_roi_group` (line 464) | `metadata, group_label, roi_names, logic='include', t_threshold=None` | `excl_idx`; call `isin(...)`; call `logical_not(...)` | No docstring; infer behavior from name/signature before reuse. |
...
| `load_model_and_fmri` (line 63) | `config: dict` | `real_features`, `fmri_dict` | No docstring; infer behavior from name/signature before reuse. |
| `prepare_inputs` (line 70) | `config: dict, real_features: np.ndarray, fmri_dict: dict` | `encoder`, `selected_features` | No docstring; infer behavior from name/signature before reuse. |
| `run_suppression_pipeline` (line 94) | `config: dict, selected_features: np.ndarray, encoder, verbose: bool=True` | `ca`, `pid`, `mi` | No docstring; infer behavior from name/signature before reuse. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `gauss_simple_example` (line 23) | `N=1000, P=1, rng_seed=1, noise_seed=1, simple_example=True, snr=1.0, method='ridge_cv', mixing_dimension=None` | `vp_results`, `pid` | Run a simple Gaussian example with no mixing deminsion each experiement has different random seeds. |
...
| `test_suppression` (line 64) | `N=1000, P=1, suppression_strength=0.5, rng_seed=1, mode='simple', snr=1.0, method='ridge_cv', mixing_dimension=None` | `vp_results`, `pid`, `mi` | Run a simple Gaussian example with no mixing deminsion each experiement has different random seeds. |
...
| `theoretical_covariance_multivariate` (line 52) | `Sigma_R, Sigma_U, Sigma_N, Sigma_eps, order=('X1', 'X2', 'Y')` | `Sigma_full`, `cov_blocks` | Theoretical covariance for the multivariate generative process: Y = R + U + eps X1 = R + U + N X2 = R + N where R ~ N(0, Sigma_R) U ~ N(0, Sigma_U) N ~ N(0, Sigma_N) eps ~ N(0, Sigma_eps) and all latent variables are mutually independent. |
| `simulate_multivariate_process` (line 123) | `n, Sigma_R, Sigma_U, Sigma_N, Sigma_eps, rng=None` | `X1`, `X2`, `Y` | Simulate: Y = R + U + eps X1 = R + U + N X2 = R + N |
| `extract_covariance_blocks` (line 161) | `Sigma_full, p` | `blocks` | Extract covariance blocks assuming order [X1, X2, Y]. |
| `whiten_theoretical_covariance_blocks` (line 187) | `Sigma_full, p, device='cpu', dtype=torch.float64` | dict (Sigma_X1X1, Sigma_X2X2, Sigma_YY, Sigma_X1X2, Sigma_X1Y, Sigma_X2Y) | Given the full covariance matrix of [X1, X2, Y], compute the whitened cross-covariance blocks: P = whitened Cov(X1, X2) Q = whitened Cov(X1, Y) R = whitened Cov(X2, Y) using whiten_block from PID_util. |
| `validate_multivariate_covariance` (line 281) | `n=1000000, p=5, seed=123, device='cpu'` | dict (Sigma_R, Sigma_U, Sigma_N, Sigma_eps, Sigma_theoretical, Sigma_empirical) | Validate the theoretical covariance by simulation, then compute whitened covariance blocks. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `bern_pair_as_channels` (line 8) | `p1=0.5, p2=0.5` | `PX1gS`, `PX2gS`, `PS` | Convert two independent Bernoulli(p1), Bernoulli(p2) into PXgS, PYgS, PS format compatible with computeQUI_numpy. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `generate_correlated_features` (line 12) | `n, p, rho, rng` | call `multivariate_normal(...)` | Generate samples with an AR(1)-style covariance structure. |
| `_apply_mixing` (line 19) | `rng, X_M1, X_M2, mixing_dimension` | X_M1 @ mixing_matrix_M1, X_M2 @ mixing_matrix_M2; `X_M1`, `X_M2` | No docstring; infer behavior from name/signature before reuse. |
| `_split_signal_sources` (line 28) | `rng, n, p, snr` | `X_M1`, `X_M2`, `target` | No docstring; infer behavior from name/signature before reuse. |
| `_feature_correlation_sources` (line 50) | `rng, n, p, snr, rho` | `X_M1`, `X_M2`, `target` | No docstring; infer behavior from name/signature before reuse. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `unq2_zero_with_red_unq1_syn` (line 15) | `rng, n, p, noise_std=0.9` | `X_M1`, `X_M2`, `y` | Continuous Gaussian-like example where theoretically: unq2 = 0 redundancy > 0 unq1 > 0 synergy > 0 The output dimensions are 3p: block 1: redundant information block 2: unique information for X1 block 3: synergistic/suppressor information |
| `run_experiment` (line 91) | `rng, suppresion_strength, mode='permuted', n=1024, p=100, mixing_dimension=None, snr=10.0, method='standard', show_diagnostic_plots=False` | `decomp`, dict (X_M1, X_M2, y) | Run commonality analysis experiment. |
