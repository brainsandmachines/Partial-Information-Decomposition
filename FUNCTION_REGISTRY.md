...
| `broja_venkatesh_bias (line 124)` | config | float | Calculate either raw Venkatesh `obj` or matching raw synergy `I(T;X1,X2)-obj` for permutation samples. Selection uses `config['pid_bias_term']`, defaults to `obj`, disables Venkatesh's multiplicative factor, and uses `[T,X1,X2]` order. |
...
| `parametric_bootstrap_obj_debias (line 293)` | config: dict, covariance=None, raw_obj: float \| None=None, statistic: str='obj' | dict | Estimate raw Venkatesh-objective or raw-synergy bias with one unrestricted fitted-Gaussian parametric-bootstrap implementation. `statistic='syn'` uses `I(T;X1,X2)-obj`, so no bounded/clipped PID output is used. |
| `lorenz_gaussian_obj_debias (line 638)` | config: dict, covariance=None, raw_obj: float \| None=None | dict | Implement the Lorenz et al. Gaussian merged correction for ordinary `ddof=1` sample covariance. Reuses exact Wishart MI bias, Gaussian parametric resampling, and target-only permutation; merges the two synergy biases, returns `obj_bias = joint_MI_bias - merged_synergy_bias`, corrected raw `obj`, corrected MI/PID values, surrogates, and diagnostics without Venkatesh proportional correction or clipping. |
| `equal_direct_wishart_control_obj_debias (line 1094)` | config: dict, covariance, raw_obj: float \| None=None | dict | Estimate only the additive bias of raw Venkatesh `output[4]` using a known equal-channel direct-covariance fit, full Gaussian bootstrap re-optimization, and exact Wishart mean-marginal-MI control variate. Requires `[T, X1, X2]`, `known_equal_direct_covariance=True`, and an unbiased sample covariance; does not correct returned MIs, pool the final objective, clip, or impose PID bounds. Task-specific oracle for the theoretical direct covariance family. |
| `equivalent_channels_obj_debias (line 1474)` | config: dict, covariance=None, raw_obj: float \| None=None | dict | Replace the nonregular objective with pooled Wishart-corrected marginal MI under explicitly known equivalent channels. This is a pooled oracle rather than a correction retaining raw optimized `obj`; do not use for objective-only Venkatesh debiasing. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `load_config (line 39)` | config_path: str \| Path=DEFAULT_CONFIG_PATH | Annotated: `dict` | Load configuration from YAML file. |
| `csv_save (line 45)` | config: dict, experiment_name: str, method: str, theoretical_values: tuple, sampled_values: dict | Annotated: `Path` | Save theoretical and sampled PID/MI values, including raw `obj`, corrected `union_info`, and their difference `obj_bias`, to one method CSV. |
| `simulation (line 81)` | config: dict, methods: list, experiment_name: str \| None=None | Annotated: `dict` | Run theoretical-covariance PID calculations and sampled trial summaries. For the constrained objective route it passes the reordered sample covariance, asserts sample-covariance status only per trial, and uses distinct bootstrap seeds. |
