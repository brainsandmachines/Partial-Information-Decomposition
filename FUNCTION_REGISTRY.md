...
| `main (line 197)` | No inputs | Annotated: `None` | Load `pc_function_config.yaml` beside the module, resolve all configured pipeline functions and repository-relative paths, run every configured model pair, save pair pickles, and generate the absolute and trivariate-MI-normalized plots. Task-specific entry point. |
