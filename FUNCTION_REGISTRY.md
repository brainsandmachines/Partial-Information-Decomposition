...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `RidgeModelArtifacts class (line 58)` | model_name: str, layer_index: int, alphas_path: Path, alphas: np.ndarray | `RidgeModelArtifacts` | Immutable task-specific record for one model's validated best layer, alpha archive, and per-target-PC alpha vector. No scaler artifact is required. |
| `to_deepdive_model_name (line 77)` | model_name: str | Annotated: `str` | Convert stored CLIP aliases at the DeepDive loading boundary while keeping artifact/result names unchanged. |
| `safe_model_name (line 93)` | model_name: str | Annotated: `str` | Replace forward and backward path separators exactly as ridge artifact generation does. |
| `_resolve_project_path (line 106)` | path_value: str \| Path | Annotated: `Path` | Resolve repository-relative configuration paths from the checkout root while preserving absolute paths. |
| `_load_or_create_checkpoint (line 122)` | output_path: Path | Annotated: `pd.DataFrame` | Load the exact 19-column resume CSV or create an empty checkpoint with that schema. Task-specific. |
| `_unfinished_unordered_pairs (line 153)` | source_1_names: list[str], source_2_names: list[str], existing_results: pd.DataFrame | Annotated: `list[tuple[str, str]]` | Remove self-pairs, completed pairs, and reverse duplicates before expensive loading while preserving the first X1/X2 orientation. |
| `_required_models (line 189)` | pairs: list[tuple[str, str]] | Annotated: `list[str]` | De-duplicate models from unfinished pairs in first-use order. |
| `_model_artifact_path (line 209)` | artifact_config: Mapping[str, Any], model_name: str, artifact_label: str | Annotated: `Path` | Fill model/safe-model templates, resolve them from the checkout root, and require the resulting artifact file. |
| `_validate_model_artifacts (line 259)` | model_names: list[str], config: Mapping[str, Any], expected_target_dim: int | Annotated: `dict[str, RidgeModelArtifacts]` | Resolve best layers and validate alpha model/layer/PC metadata before feature extraction. The raw-feature ridge path does not resolve source scalers. |
| `_load_model_context (line 334)` | model_name: str | Annotated: `tuple[dict[str, Any], float]` | Load one DeepDive model context, discover its ordered layers, and report model-labelled timing and failures. |
| `_release_model_context (line 362)` | model_context: dict[str, Any] \| None | Annotated: `None` | Clear one model context, collect unreachable objects, and release unused CUDA cache. |
| `_prepare_ridge_prediction (line 379)` | model_name: str, artifacts: RidgeModelArtifacts, target_context: dict[str, Any], train_target: np.ndarray, shared_mask: np.ndarray, feature_extraction: Callable[..., Any], feature_extraction_kwargs: Mapping[str, Any], *, seed: int | Annotated: `tuple[np.ndarray, int]` | Load and validate one model context, extract its aligned raw features, release the model, ridge-fit only non-shared rows without variance standardization, and return its held-out prediction plus selected layer. Task-specific and safe to run in the single background worker. |
| `_iter_ridge_prediction_pairs (line 456)` | pairs: list[tuple[str, str]], artifacts_by_model: Mapping[str, RidgeModelArtifacts], target_context: dict[str, Any], train_target: np.ndarray, shared_mask: np.ndarray, feature_extraction: Callable[..., Any], feature_extraction_kwargs: Mapping[str, Any], *, seed: int, prefetch_ridge_predictions: bool | Annotated: `Iterator[tuple[str, str, np.ndarray, int, np.ndarray, int]]` | Yield PID-ready ordered pairs from cached held-out predictions and, before each yield, submit the next unseen model's complete load/extract/ridge preprocessing to one worker so it overlaps the caller's current PID calculation. Model features remain in their original units. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `_prepare_source_for_pid (line 32)` | source: np.ndarray, train_target: np.ndarray, shared_mask: np.ndarray, ridge: bool | Annotated: `np.ndarray` | Return held-out source features directly, or fit ridge on non-shared rows after the caller has selected the current cumulative target-PC subset and return held-out predictions for exactly that subset. Task-specific; no variance standardization is added here. |
| `_save_pair_results (line 58)` | pair_results: dict[int, dict[str, Any]], model_1: str, model_2: str, results_dir: str \| Path | Annotated: `Path` | Pickle one model pair's complete cumulative target-PC PID/MI result mapping using filesystem-safe model names. Task-specific. |
| `pc_function_analysis (line 86)` | config: dict[str, Any], functions: PIDPipelineFunctions, model1_name: list[str], model2_name: list[str], pc_path: str \| Path, hdf_path: str \| Path, pkl_info_path: str \| Path, neural_data_path: str \| Path, results_dir: str \| Path \| None=None, plot_dir: str \| Path \| None=None | Annotated: `dict[str, dict[str, dict[int, dict[str, Any]]]]` | Prepare the target and raw sources, then for each cumulative target-PC count first select those target PCs, fit both ridge models specifically to that subset, predict held-out rows, and calculate PID/MI. Immediately after each model pair finishes, optionally save its pickle and both plots before continuing to the next pair. |
| `main (line 209)` | No inputs | Annotated: `None` | Load `pc_function_config.yaml` beside the module, resolve all configured pipeline functions and repository-relative paths, and run every configured model pair while `pc_function_analysis` saves that pair's pickle and plots immediately on completion. Task-specific entry point. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `find_alpha_per_pc (line 25)` | predictor: np.ndarray, target: np.ndarray | Annotated: `tuple[np.ndarray, Pipeline]` | Fit one multi-target `RidgeCV` directly on raw model features, with an intercept and one selected alpha per target PC. Returns the alpha vector and fitted raw-input pipeline; no predictor variance scaler is fitted or returned. Task-specific; predictor and target rows must align. |
| `load_and_apply_pca (line 102)` | data: np.ndarray, pca_path: str \| Path | Annotated: `np.ndarray` | Apply a saved centered PCA model directly to raw neural responses without refitting and without variance standardization. |
| `split_alphas_csv_by_model (line 124)` | alphas_csv_path: str \| Path, output_dir: str \| Path \| None=None | Annotated: `list[Path]` | Validate and split the aggregate model/layer/PC/alpha CSV into one model-specific `.npz` archive, preserving ordered one-based PC indices and sanitizing model path separators. |
| `main (line 246)` | source_name: str, path_to_results: str \| Path, pc_path: str \| Path, hdf_path: str \| Path, pkl_info_path: str \| Path, neural_data_path: str \| Path, alphas_csv_path: str \| Path | Annotated: `tuple[np.ndarray, Pipeline]` | Extract aligned unique-image raw model features, project raw neural targets with centered PCA, fit per-PC ridge penalties without variance standardization, and append one-based alpha rows to a validated four-column CSV. Task-specific; rerunning the same model appends duplicate rows. |
| `check_path_exists (line 374)` | config: dict[str, Any] | Annotated: `None` | Recursively require configured input `Path` values before command-line alpha generation. Defined only in the module's script entry point. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `split_unique_shared (line 27)` | subj_id: str, hdf_path: str \| Path, pkl_info_path: str \| Path, neural_data_path: str \| Path, variance_threshold: float=0.99 | Annotated: `dict[str, Any]` | Load a subject context and use its one-dimensional `shared1000_subj` mask to split aligned raw target rows and subject image IDs. Task-specific; `unique_neural_data` is training data and `shared_neural_data` is held out. |
| `pca_by_variance (line 90)` | neural_data: np.ndarray, variance_threshold: float=0.99 | Annotated: `dict[str, Any]` | Fit full-SVD PCA directly on two-dimensional raw training samples, retaining the requested training-variance fraction. Scikit-learn PCA mean-centers columns but does not divide them by standard deviation; `1.0` retains all valid components. |
| `heldout_pca (line 128)` | pca_model: PCA, heldout_data: np.ndarray | Annotated: `np.ndarray` | Apply the fitted centered PCA basis directly to aligned raw held-out samples without refitting or variance standardization. |
| `pca_func (line 154)` | data: np.ndarray, mode: str='eigenvector_CV', max_features: int \| None=None, variance_threshold: float=0.99 | Annotated: `dict[str, Any]` | Select a target component count using the variance, eigenvector-CV, or missMDA-CV route and fit centered PCA directly on raw data. The missMDA selector is called with `scale=False`. |
| `main (line 223)` | subj_id: str, hdf_path: str \| Path, pkl_info_path: str \| Path, neural_data_path: str \| Path, variance_threshold: float=0.99, save_models_path: str \| Path \| None=None, max_features: int \| None=None, pca_mode: str='missmda_CV' | Annotated: `dict[str, Any]` | Run centered, unstandardized subject PCA and optionally save the PCA model plus a per-PC held-out variance CSV. Held-out ratios use total raw held-out sample variance; PC indices are one-based. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `pca_projection (line 23)` | features, n_components | `ft_reduced` | Apply PCA projection to reduce dimensionality of features. |
| `jl_projection (line 40)` | features, n_samples, eps=0.1, jl_dim=None | `ft_reduced` | Apply Johnson-Lindenstrauss projection to reduce dimensionality of features. |
| `cca_projection (line 73)` | features1, features2, n_components | tuple of 2 values | Apply Canonical Correlation Analysis (CCA) to find linear combinations of two sets of features that are maximally correlated. This separate helper standardizes its own CCA inputs and is not used by the ridge pairwise path. |
| `prepare_ridge_target (line 106)` | target: np.ndarray, target_context: Mapping[str, Any], pc_target_path: str \| Path | Annotated: `tuple[np.ndarray, np.ndarray, np.ndarray]` | Transform raw neural responses with their saved centered PCA, then return non-shared training scores, shared held-out scores, and a copied aligned Boolean mask. Does not fit PCA or variance-standardize the target. |
| `load_ridge_alphas (line 189)` | alphas_path: str \| Path, *, model_name: str, expected_target_dim: int, expected_layer_index: int \| None=None | Annotated: `np.ndarray` | Load one model's alpha archive and validate required fields, exact model/layer metadata, finite nonnegative values, and ordered one-based target-PC indexes. |
| `ridge_predict_shared (line 314)` | source: np.ndarray, train_target: np.ndarray, shared_mask: np.ndarray, alphas: np.ndarray, *, seed: int | Annotated: `np.ndarray` | Fit multi-output ridge only on non-shared raw source rows and return finite shared-row predictions in target-PC space, preserving a singleton target axis. Ridge fits an intercept but performs no variance standardization. |
| `feature_manipulation_ridge (line 418)` | source1: np.ndarray, source2: np.ndarray, target: np.ndarray, target_context: Mapping[str, Any], seed: int, model_name_1: str, model_name_2: str, pc_target_path: str \| Path, alphas_source1_path: str \| Path, alphas_source2_path: str \| Path | Annotated: `tuple[np.ndarray, np.ndarray, np.ndarray]` | Backward-compatible grouped ridge manipulation composed from the raw-target and raw-source helpers; returns held-out X1 prediction, X2 prediction, and measured target in that order without variance standardization. |
...
| `broja_venkatesh_bias (line 124)` | config | float | Calculate either raw Venkatesh `obj` or the plugin gPID synergy for permutation samples. Selection uses `config['pid_bias_term']`, defaults to `obj`, disables Venkatesh's multiplicative factor, and uses `[T,X1,X2]` order. |
| `permutation_null_debias (line 169)` | config, func | dict (bias, perm_mean, perm_std, perm_se, perm_values, n_perm); dict (debiased, perm_mean, perm_std, perm_se, perm_values, n_perm) | Debias an MI-like estimator by subtracting its permutation null floor. |
...
| `parametric_bootstrap_obj_debias (line 292)` | config: dict, covariance=None, raw_obj: float \| None=None, statistic: str='obj' | dict | Estimate raw Venkatesh-objective or plugin-synergy bias with one unrestricted fitted-Gaussian parametric-bootstrap implementation. `statistic='syn'` uses the estimator's plugin gPID synergy, matching the Lorenz procedure. |
| `lorenz_gaussian_obj_debias (line 637)` | config: dict, covariance=None, raw_obj: float \| None=None | dict | Implement the Lorenz et al. Gaussian merged correction for ordinary `ddof=1` sample covariance. Reuses exact Wishart MI bias, Gaussian resampling, and target-only permutation to correct plugin synergy and union. Returns the total amount `raw_obj - corrected_union`, corrected MI/PID values, and surrogate values; uses `obj_bootstrap_seed` as the per-dataset root-seed fallback, does not use Venkatesh's proportional correction, and does not clip corrected outputs. |
| `equal_direct_wishart_control_obj_debias (line 965)` | config: dict, covariance, raw_obj: float \| None=None | dict | Estimate only the additive bias of raw Venkatesh `output[4]` using a known equal-channel direct-covariance fit, full Gaussian bootstrap re-optimization, and exact Wishart mean-marginal-MI control variate. Requires `[T, X1, X2]`, `known_equal_direct_covariance=True`, and an unbiased sample covariance; does not correct returned MIs, pool the final objective, clip, or impose PID bounds. Task-specific oracle for the theoretical direct covariance family. |
| `equivalent_channels_obj_debias (line 1345)` | config: dict, covariance=None, raw_obj: float \| None=None | dict | Replace the nonregular objective with pooled Wishart-corrected marginal MI under explicitly known equivalent channels. This is a pooled oracle rather than a correction retaining raw optimized `obj`; do not use for objective-only Venkatesh debiasing. |
...
| `make_both_unique_true_cov_from_config (line 183)` | config: dict, rng: torch.Generator \| None=None | Annotated: `torch.Tensor` | Construct the Venkatesh/Lorenz Gaussian population covariance in `[X1,X2,T]` order from independent Bernoulli source-target channels and independent unit source noise. `both_unique_connection_probability` selects the channel probability, defaults to the paper's 0.1, and gives an exact zero-information target null at 0.0. |
...
| Function / Method | Inputs | Outputs | What it does |
|---|---|---|---|
| `load_config (line 45)` | config_path: str \| Path=DEFAULT_CONFIG_PATH | Annotated: `dict` | Load configuration from YAML file. |
| `csv_save (line 51)` | config: dict, experiment_name: str, method: str, theoretical_values: tuple, sampled_values: dict | Annotated: `Path` | Save theoretical and sampled PID/MI values, including raw `obj`, corrected `union_info`, and their difference `obj_bias`, to one method CSV. |
| `simulation (line 87)` | config: dict, methods: list, experiment_name: str \| None=None | Annotated: `dict` | Run theoretical-covariance PID calculations and sampled trial summaries. `use_both_unique_covariance=True` selects the Venkatesh/Lorenz both-unique covariance; otherwise the existing direct covariance is used. For the constrained objective route it passes the reordered sample covariance, asserts sample-covariance status only per trial, and uses distinct bootstrap seeds. |
