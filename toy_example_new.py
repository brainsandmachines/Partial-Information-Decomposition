...
def compute_ols_cv_r2(X, y):
    """
    Compute cross-validated RÂ² using leave-one-out cross-validation.
    ...
    Returns:
        float: Cross-validated RÂ² (can be negative if model overfits badly)
    """
...
def compute_ridge_cv_r2(X, y, alphas=None):
    """
    Compute cross-validated RÂ² using RidgeCV with efficient LOO cross-validation.
    ...
    Returns:
        float: Best cross-validated RÂ² across all alpha values.
    """
    if alphas is None:
...
def compute_r2(X, y):
    """
    Compute in-sample RÂ² for OLS regression.
...
def commonality_analysis(features_A, features_B, target, method='standard', alphas=None):
    ...
    - 'standard': In-sample RÂ² (prone to overfitting)
    - 'ols_cv': Cross-validated RÂ² using PRESS residuals
    - 'ridge_cv': Cross-validated RÂ² using RidgeCV with GCV
    ...
    Args:
        ...
        method (str): Which RÂ² computation method to use: 'standard', 'ols_cv', or 'ridge_cv'.
        alphas (array-like, optional): Alpha values for RidgeCV (only used if method='ridge_cv').
        
    Returns:
        dict: A dictionary with RÂ² values and variance decomposition.
    """
    n = len(target)
    ...
    # Select RÂ² computation function based on method
    if method == 'standard':
...
def run_experiment(rng, n=1024, p=100, mixing_dimension=None, snr=10.0, method='standard'):
    ...
    Args:
        ...
        method: Which RÂ² computation to use: 'standard', 'ols_cv', or 'ridge_cv'
    ...
    #half of the features are "real" (contribute to target), half are "spurious"
    ...
    #noisy_signal = signal + noise_std * rng.standard_normal(n)
    ...
    if mixing_dimension is not None:
        ...
        mixing_matrix_M1 = rng.standard_normal((p, mixing_dimension))
        X_M1 = X_M1 @ mixing_matrix_M1
        mixing_matrix_M2 = rng.standard_normal((p, mixing_dimension))
    ...
    # Verify sum (only variance components, not RÂ² values)
    # Note: CV version may not sum exactly due to negative RÂ² being possible
    if method == 'standard':
...
def run_all_methods(rng_seed, n, p, mixing_dimension, snr):
    ...
    for method in ['standard', 'ols_cv', 'ridge_cv']:
        ...
        _, is_common_negative = run_experiment(rng, n=n, p=p, mixing_dimension=mixing_dimension, snr=snr, method=method)
        common_negative.append(is_common_negative)
    if all(common_negative):
        print("\nAll common variance estimates are negative âœ…")
        print(f"With mixing_dimension={mixing_dimension} and snr={snr},\n We succesfully broke variance partitioning â—â—â—.") 

    else:
        print("\nNot all common variance estimates are negative âŒ")
...
def main():
    ...
    print("\n" + "="*70)
    print("Experiment 2: LOW SNR + INVERTIBLE MIXING (200â†’200)")
    print("="*70)
    run_all_methods(SEED, N, P, mixing_dimension=200, snr=1.0)

    print("\n" + "="*70)
    print("Experiment 3: LOW SNR + LOSSY MIXING (200â†’100)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("Experiment 5: HIGH SNR + INVERTIBLE MIXING (200â†’200)")
    print("="*70)
    run_all_methods(SEED, N, P, mixing_dimension=200, snr=10.0)

    print("\n" + "="*70)
    print("Experiment 6: HIGH SNR + LOSSY MIXING (200â†’100)")
    print("="*70)
