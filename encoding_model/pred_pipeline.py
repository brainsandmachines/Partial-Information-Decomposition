...
from encoding_utils import visualize_encdoing_accuaracy,save_corellation,save_model
...
def pipeline(data_dir, parent_submission_dir, subj,args,layer_name='features.2',model=None,features=None, only_validate=False):
    ...
    if not only_validate:
        ...
        reg_lh, reg_rh, lh_correlation, rh_correlation = encoding_model_instance.run_model(train_imgs_dataloader,val_imgs_dataloader,lh_fmri_train,
                                                                                    rh_fmri_train,lh_fmri_val,rh_fmri_val,batch_size=500,ncomponents=100)
        features_val_pred_lh = encoding_model_instance.lh_fmri_val_pred
        features_val_pred_rh = encoding_model_instance.rh_fmri_val_pred
        features_train = encoding_model_instance.features_train
        features_val_trained = encoding_model_instance.features_val
        #save_fmri
    ...
    return  {'reg_lh': reg_lh, 'reg_rh': reg_rh, 'lh_correlation': lh_correlation, 'rh_correlation': rh_correlation,'features_val_pred_lh': features_val_pred_lh, 'features_val_pred_rh': features_val_pred_rh,'features_train': features_train,'features_val_trained': encoding_model_instance.features_val if not only_validate else None, 'predict_array': predict_array if only_validate else None}


def trained_model(layer_name,model,model_name):
    output_dict = pipeline(data_dir, parent_submission_dir, subj,args,layer_name,model=model)
    ...
    model_name = f'model_{model_name}_{layer_name}_subj{subj}'

    models_folder = save_model(reg_lh, reg_rh,features_val_pred_lh,features_val_pred_rh,features_train,features_val_trained,folder_path=trained_model_dir, model_name=model_name,predict_array=None)
...
if __name__ == "__main__":
    ...
    trained_model(layer_name, model,model_name=model)
