...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run gpid delta PID on a covariance CSV.")
...
def result_row(values: tuple[float, ...]) -> dict[str, object]:
    imx, imy, imxy, _defx, _defy, uix, uiy, ri, si = values[:9]
    return {
        "case": "EvilTwin",
...
def main() -> int:
    try:
        ...
        values = estimate.approx_pid_from_cov(cov, nt, n1, n2, verbose=False)
