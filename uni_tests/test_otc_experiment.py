...
def test_run_otc_experiment_overall_best_removes_invisible_model_chars(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Check overall-best lookup tolerates invisible format characters.
    ...
    register_dummy_functions(monkeypatch)
    csv_path = tmp_path / "overall_best_with_invisible_chars.csv"
    csv_path.write_text(
        "model_name,best_layer_name,best_layer_index,mean_cv_corr,test_corr\n"
        "\ufeffM1,b,3,0.1,0.2\n"
        "M2\u200b,h,4,0.3,0.4\n",
