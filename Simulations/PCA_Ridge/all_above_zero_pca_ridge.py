...
if __name__ == "__main__":
    ...
    unique1_weight, unique2_weight, redundant_weight, shared_noise_weight = 6.0, 7.0, 1.0, 1.0
    if n_trials < 1:
    ...
    all_above_zero_covariance = torch.tensor([
    ...
    ground_truth_nats = calculate_mi_raw(torch.device("cpu"), all_above_zero_covariance, [1, 1, 1])
    ...
    pid_covariance = all_above_zero_covariance[covariance_order][:, covariance_order].to(pid_dtype)  # [X1, X2, T] -> [T, X1, X2]
    ...
    ground_truth_cmi_per_dimension = calculate_covariance_cmi(all_above_zero_covariance, [1, 1, 1])
    ...
    for trial in range(n_trials):
        ...
        raw_source1, raw_source2, raw_target = all_above_zero_weighted(
            np.random.default_rng(trial_seed), n_samples, p, noise_std,
            unique1_weight, unique2_weight, redundant_weight, shared_noise_weight,
    ...
    for route, dimensions in route_dimensions.items():
        ...
        print(f"\n{'=' * 10} ALL ABOVE ZERO — {route} — MEAN OF {n_trials} TRIALS {'=' * 10}")
        print_pid_mi(
    ...
    save_sample_simulation_results_table(
        ...
        plot_path,
        title="All Above Zero PID: RAW vs PCA vs Ridge CV",
    )
