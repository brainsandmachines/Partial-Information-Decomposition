"""Compatibility wrapper for regular RVs_Story examples."""
...
from story_pid_utils import run_pid_story, truth_pid_equal_unique


def true_mi_pid(sources, target, covariance=None):
    """Return the equal-unique true PID row.
    ...
    """
    return truth_pid_equal_unique(sources, target, covariance=covariance)


def main_func(config, function_to_run):
    """Run a regular RV generator through the shared PID story runner.
    ...
    """
    return run_pid_story(config, function_to_run, truth_func=truth_pid_equal_unique)
